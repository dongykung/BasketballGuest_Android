...
class ChatRoomRepositoryImpl @Inject constructor(
    ...
    override fun listenToChatRooms(myUid: String): Flow<Unit> = callbackFlow {
        ...
        val listenerRegistration = query.addSnapshotListener { snapshot, error ->
            ...
            for (diff in snapshot.documentChanges) {
                val chatRoomDTO =
                    diff.document.toObject(ChatRoomDTO::class.java).copy(id = diff.document.id)
                        ?: continue
                when (diff.type) {
                    DocumentChange.Type.ADDED -> {
                        CoroutineScope(context = Dispatchers.IO).launch {
                            val myUnreadCount = getCountUnReadMessage(
                                uid = myUid,
                                chatRoomId = diff.document.id,
                                date = chatRoomDTO.readStatus[myUid] ?: Date()
                            )
                            val otherUserUid = chatRoomDTO.participant.first { it != myUid }
                            val otherUserUnreadCount = getCountUnReadMessage(
                                uid = otherUserUid,
                                chatRoomId = diff.document.id,
                                date = chatRoomDTO.readStatus[otherUserUid] ?: Date()
                            )
                            val chatEntity = chatRoomDTO.toChatRoomEntitiy()
                            chatEntity.unReadCount =
                                mapOf(myUid to myUnreadCount, otherUserUid to otherUserUnreadCount)
    ...
    override suspend fun createChatRoom(chatRoom: ChatRoom): Result<Unit> = kotlin.runCatching {
